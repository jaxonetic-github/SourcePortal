...
class CalendarView extends Component {
  constructor(props) {
    ...
    if (this.props.route) {
      this.state = {
        ...
        markedDates: [],
        selected: this.props.route.params.initialDate,
      };
    } else if (props && props.generalView) {
  ...
  calendarFilteredData = (selectedDate) => {
    ...
    const filteredDates = this.props.events.map((event) => {
      ...
      return event.calendar.month && event.calendar.month === month
        ? this.createMarkedDate(event.calendar)
        : null;
    });
  ...
  render() {
    ...
    if (!this.props.generalView) {
      //      console.log(this);
      let selectedEvent = this.props.events.filter(
        (evt) => evt.id == this.state.key,
      );
      test =
        selectedEvent[0].calendar.year +
        '-' +
        selectedEvent[0].calendar.month +
        '-' +
        selectedEvent[0].calendar.day;
    }
    ...
    for (
      let i = 0;
      filteredMarkedDates[i] && i < filteredMarkedDates.length;
      i++
    ) {
      //  console.log(filteredMarkedDates[i]);
      if (filteredMarkedDates[i].formattedDate == test) {
        markedDates_ex[filteredMarkedDates[i].formattedDate] = {
          ...filteredMarkedDates[i].dateStyle,
          selectedColor: 'maroon',
        };
      } else {
        markedDates_ex[filteredMarkedDates[i].formattedDate] = {
          ...filteredMarkedDates[i].dateStyle,
          selectedColor: COMMON_DARK_BACKGROUND,
        };
      }
    }
  ...
  onDayPress = (day) => {
    if (!this.props.generalView) {
      this.props.updateEventCalendarByKey({
        payload: {
          year: moment(day.dateString).format('YYYY'),
          month: moment(day.dateString).format('MM'),
...
function matchDispatchToProps(dispatch) {
  return bindActionCreators(
    {updateEventCalendarByKey: updateEventCalendarByKey},
    dispatch,
  );
}
