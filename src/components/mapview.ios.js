...
*/
  constructor(props) {
    ...
    this.state = {
      ...
      location: this.props.route.params.initialLocation,
    };
  }
  ...
  async componentDidMount() {
    const tmp = await navigator.geolocation.getCurrentPosition(
      (position) => {
        //{"coords":{"speed":-1,"longitude":-122.406417,"latitude":37.785834,"accuracy":5,"heading":-1,"altitude":0,"altitudeAccuracy":-1},"timestamp":1551589085878.46}
  ...
  onAddressChange = async (region) => {
    try {
      ...
      this.props.updateEventLocationByKey({
        key: this.state.key,
        location: region,
      });
...
*/
  ...
  render() {
    console.log(this.state);
    const locations = LOCATION_LIST.map((event) => (
      <Picker.Item key={event.label} label={event.label} value={event.value} />
    ));
    const states = STATES.states.map((event) => (
    ...
    const formattedAddress = this.state.position
            ...
            <Button
              ...
              onPress={() => {
                this.props.history.push(
                  '/Activities/EventView/' + this.state.key,
                );
              }}>
              <Text>Back></Text>
            </Button>
...
function matchDispatchToProps(dispatch) {
  return bindActionCreators(
    {updateEventLocationByKey: updateEventLocationByKey},
    dispatch,
  );
}
